package com.lxf.thread.threadpool;
...
public class ThreadPoolImpl implements ThreadPool {
    ...
    private static ThreadPoolImpl threadPool;
    ...
    private ThreadPoolImpl(int workerNum2) {
        if (workerNum2 > 0) {
        ...
        for (int i = 0; i < workerNum; i++) {
            ...
            System.out.println("初始化线数: " + (i + 1) + "/" + (workerNum) + "  ----当前线程名称是：" + workThreads[i].getName());
            workThreads[i].start();
        }
    }
...
//    }
    ...
    public static ThreadPool getThreadPool() {
        return getThreadPool(ThreadPoolImpl.workerNum);
    }

    public static ThreadPool getThreadPool(int workerNum) {
        if (workerNum <= 0) {
            workerNum = ThreadPoolImpl.workerNum;
        }
        if (threadPool == null) {
